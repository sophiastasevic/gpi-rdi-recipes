...
# -*- coding: utf-8 -*-
"""
Created on Fri May  3 11:18:07 2024
...
def get_paths(sofname, data_names):
    ...
    count_signal = 0 ## counter for cubes that are removed because they have a disk/bianry/planet
    for path in data_paths['cube']:
        try:
            ...
            ## target has known signal, so do not include in ref list
            if star_data.loc[sid, ['binary','planet','disk']].any() == True:
                count_signal += 1
            
            ## add to list of reference cubes
            else:
                ref_paths['cube'].append(path)
    ...
    print(count_signal, "reference target(s) removed for having known disks/companions")
    
    ref_data = pd.DataFrame(target_data, index=pd.Index(ref_paths['cube'], name='path'))
    ref_data.rename_axis(columns={'naxis3':'nframes'})
    ref_data.to_csv("ref_target_data.csv", index=False)
    
    ref_paths['frame'] = match_frame_select(data_paths['frame'], ref_data['date-obs'], ref_data['nframes'])
    
    return ref_paths, ref_data
#%%
"""
---------------------------------- MAIN CODE ----------------------------------
"""
if __name__=='__main__':
    ...
    parser.add_argument('--normalise', action='store_true', help='Divide each frame by its standard deviation before saving.')
    parser.add_argument('--stack_wl', action='store_true', help='Mean combine frames along spectral channels.')
    ...
    ## collapse cube along wavelength
    STACK = args.stack_wl
    
    data_names = {'cube':('GPI_SCIENCE_REDUCED_SPECTRAL_MASTER_CUBE', 1),
                  'frame':('GPI_FRAME_SELECTION_VECTOR', 0)}
    ref_paths, ref_data = get_paths(sofname, data_names)
    ncubes = len(ref_paths['cube'])
    
    ## initialise header
    hdr = fits.Header()
    hdr['NB_REF_CUBES'] = ncubes
    hdr_tmp = fits.getheader(ref_paths['cube'][0])
    for k in ['PIXTOARC', 'ESO INS COMB IFLT', 'ESO INS2 COMB IFS']:
        hdr[k] = (hdr_tmp[k], hdr_tmp.comments[k])
    hdr.set('', '----------------')
    hdr.set('COMMENT', "object header indexing starts at 1 to match ds9 indexing")
    
    nxy, nchannel = [hdr_tmp['NAXIS%d' % i] for i in [1,4]]
    if crop == None:
        crop = nxy
    elif nxy < crop:
        crop = nxy
        print("[Warning] Input crop parameter larger than original frame size, setting crop to %d" % crop)
    if (nxy+crop)%2:
        nxy+=1 ## for accurate centering
    llim, rlim = (nxy-crop)//2, (nxy+crop)//2
    
    if STACK == False:
        ## take every 3rd wavelength channel frame (cube too large otherwise)
        wli = np.arange(nchannel, step=3)
        hdr['WL_SAMPL'] = (",".join([str(wl) for wl in wli]), "Wavelength channel index kept in cube.")
    else:
        hdr['WL_STACK'] = ("mean", "method used to collapse the cube along the spectral channels")
    
    cube_sample = [] #np.zeros((ncubes,nchannel,crop,crop))
    for i, path in enumerate(ref_paths['cube']):
        obj, epoch, sid = ref_data.loc[path, ['object','date-obs','simbad main identifier']]
        cube = read_file(fits.getdata, path)
        nframes = cube.shape[1]
        if nframes < 3:
                print("[Warning] Only %d frames in %s observation of %s" % nframes, epoch, obj)
        
        ## take first, middle, and last frame of the cube and crop to specified size in x/y axis
        framei = (0,nframes//2,-1)
        cube = cube[:,framei,llim:rlim,llim:rlim]
        if STACK == True:
            ## mean combine spectral channels
            cube_sample.append(np.nanmean(cube, axis=0))
        else:
            cube_sample.append(cube[wli])
        
        nb_str = '{0:04d}'.format(i+1)
        hdr['OBJ_'+nb_str] = obj
        hdr['OBS_'+nb_str] = epoch
        hdr['SID_'+nb_str] = sid
    
    cube_sample = np.stack(cube_sample)    
    if NORM == True:
        cube_sample /= np.nanstd(cube_sample, axis=(-2,-1), keepdims=True)
        hdr['NORM'] = ("std", "method used to normalise frames")
    
    hdr['COMMENT'] = "[OBJ]= Object name from cube header"
    hdr['COMMENT'] = "[OBS]= Observing date from cube header"
    hdr['COMMENT'] = "[SID]= Simbad main identifier from RA Dec query"
        
    hdu = fits.PrimaryHDU(data=cube_sample, header=hdr)
    hdu.writeto('ref_target_frames.fits')