...
if __name__=='__main__':
    ...
    for each cube path:
        ...
        read in cube: read_file(fits.getdata, paths['cube'][i])
        
        read in frame selection vector: read_file(fits.getdata, paths['frame'][i])
        
        apply frame selection vector to cube
        (see: get_frame_data and selection_vector functions of 
              gpi_rdi_build_ref_lib/gpi_rdi_build_ref_library.py)
    
        stack cubes along temporal axis: np.nanmean(cube, axis=0)
        
        save in an array
    """
    
    """
    calculate correlation between each stacked cube and save the correlation matrix as a fits
    (see: gpi_rdi_build_ref_lib/frame_correlation.py and ird_rdi_build_corr_matrix/ird_rdi_build_corr_matrix.py)
    """